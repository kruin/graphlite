...
echo.
echo OpenGraph Lite Viewer v4482
echo.
echo Start lokale server op alle netwerkadapters: http://0.0.0.0:8088
echo PC lokaal:   http://127.0.0.1:8088/reset-cache.html?v4482
...
echo.
start "" cmd /c "timeout /t 2 /nobreak >nul && start "" http://127.0.0.1:8088/reset-cache.html?v4482"
where py >nul 2>nul
if %errorlevel%==0 (
