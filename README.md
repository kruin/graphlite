# OpenGraph Lite Viewer — PWA v4446

## v4446 — documentatie plaatsingsregels
...
## v4446 — lokale Wissel op de LEX-as
...
## v4446 — lexicon- en uitingeneditor
...
Om te voorkomen dat de browser alleen `index.html` vernieuwt maar een oude `viewer.js` houdt, laadt `index.html` nu `viewer.js?v4446` en `styles.css?v4446`.


## v4446 — OPN-slot voor vooropplaatsing
...
## v4446 — voorbeeldzinnen als HTML-input
...
## v4446 — structure-config als eerste stap
...
## v4446 — functional config: PRED apart, ARG-STRUCT apart
...
### v4446 startdiagnose
Open bij startproblemen `reset-cache.html?v4446` of `debug.html?v4446`. De debug controleert nu of de root-index daadwerkelijk de viewer is.



## v4446 — expliciete render-volgorde
...
## v4446 root-index fix
`docs/index.html` is vervangen door `docs/docs-home.html`; `viewer.html` is toegevoegd als fallback. Gebruik `debug.html?v4446` om te testen of root `index.html` echt de viewer is.

## v4446 — groei-presentatie
...
## v4446-correctie groei/projectiewissel

`LEX` is geen groei-projectie. In v4407 kon wisselen naar `LEX` de globale groeistap naar 0 zetten, waardoor `Assen`, `Bron` en `LOG/FT` leeg bleven zolang groei actief was. v4446 bewaart de laatst geldige groeistap en clamped alleen in ondersteunde groei-projecties.
...
## v4446 - beweeglijke boom/LEX-view
