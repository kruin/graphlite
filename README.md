# OpenGraph Lite Viewer — PWA v4440

## v4440 — documentatie plaatsingsregels
...
## v4440 — lokale Wissel op de LEX-as
...
## v4440 — lexicon- en uitingeneditor
...
Om te voorkomen dat de browser alleen `index.html` vernieuwt maar een oude `viewer.js` houdt, laadt `index.html` nu `viewer.js?v4440` en `styles.css?v4440`.


## v4440 — OPN-slot voor vooropplaatsing
...
## v4440 — voorbeeldzinnen als HTML-input
...
## v4440 — structure-config als eerste stap
...
## v4440 — functional config: PRED apart, ARG-STRUCT apart
...
### v4440 startdiagnose
Open bij startproblemen `reset-cache.html?v4440` of `debug.html?v4440`. De debug controleert nu of de root-index daadwerkelijk de viewer is.



## v4440 — expliciete render-volgorde
...
## v4440 root-index fix
`docs/index.html` is vervangen door `docs/docs-home.html`; `viewer.html` is toegevoegd als fallback. Gebruik `debug.html?v4440` om te testen of root `index.html` echt de viewer is.

## v4440 — groei-presentatie
...
## v4440-correctie groei/projectiewissel

`LEX` is geen groei-projectie. In v4407 kon wisselen naar `LEX` de globale groeistap naar 0 zetten, waardoor `Assen`, `Bron` en `LOG/FT` leeg bleven zolang groei actief was. v4440 bewaart de laatst geldige groeistap en clamped alleen in ondersteunde groei-projecties.
