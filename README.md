# OpenGraph Lite Viewer — PWA v4438

## v4438 — documentatie plaatsingsregels
...
## v4438 — lokale Wissel op de LEX-as
...
## v4438 — lexicon- en uitingeneditor
...
Om te voorkomen dat de browser alleen `index.html` vernieuwt maar een oude `viewer.js` houdt, laadt `index.html` nu `viewer.js?v4438` en `styles.css?v4438`.


## v4438 — OPN-slot voor vooropplaatsing
...
## v4438 — voorbeeldzinnen als HTML-input
...
## v4438 — structure-config als eerste stap
...
## v4438 — functional config: PRED apart, ARG-STRUCT apart
...
### v4438 startdiagnose
Open bij startproblemen `reset-cache.html?v4438` of `debug.html?v4438`. De debug controleert nu of de root-index daadwerkelijk de viewer is.



## v4438 — expliciete render-volgorde
...
## v4438 root-index fix
`docs/index.html` is vervangen door `docs/docs-home.html`; `viewer.html` is toegevoegd als fallback. Gebruik `debug.html?v4438` om te testen of root `index.html` echt de viewer is.

## v4438 — groei-presentatie
...
## v4438-correctie groei/projectiewissel

`LEX` is geen groei-projectie. In v4407 kon wisselen naar `LEX` de globale groeistap naar 0 zetten, waardoor `Assen`, `Bron` en `LOG/FT` leeg bleven zolang groei actief was. v4438 bewaart de laatst geldige groeistap en clamped alleen in ondersteunde groei-projecties.
