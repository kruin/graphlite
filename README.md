# OpenGraph Lite Viewer — PWA v4442

## v4442 — documentatie plaatsingsregels
...
## v4442 — lokale Wissel op de LEX-as
...
## v4442 — lexicon- en uitingeneditor
...
Om te voorkomen dat de browser alleen `index.html` vernieuwt maar een oude `viewer.js` houdt, laadt `index.html` nu `viewer.js?v4442` en `styles.css?v4442`.


## v4442 — OPN-slot voor vooropplaatsing
...
## v4442 — voorbeeldzinnen als HTML-input
...
## v4442 — structure-config als eerste stap
...
## v4442 — functional config: PRED apart, ARG-STRUCT apart
...
### v4442 startdiagnose
Open bij startproblemen `reset-cache.html?v4442` of `debug.html?v4442`. De debug controleert nu of de root-index daadwerkelijk de viewer is.



## v4442 — expliciete render-volgorde
...
## v4442 root-index fix
`docs/index.html` is vervangen door `docs/docs-home.html`; `viewer.html` is toegevoegd als fallback. Gebruik `debug.html?v4442` om te testen of root `index.html` echt de viewer is.

## v4442 — groei-presentatie
...
## v4442-correctie groei/projectiewissel

`LEX` is geen groei-projectie. In v4407 kon wisselen naar `LEX` de globale groeistap naar 0 zetten, waardoor `Assen`, `Bron` en `LOG/FT` leeg bleven zolang groei actief was. v4442 bewaart de laatst geldige groeistap en clamped alleen in ondersteunde groei-projecties.
