# OpenGraph Lite Viewer — PWA v4435

## v4435 — documentatie plaatsingsregels
...
## v4435 — lokale Wissel op de LEX-as
...
## v4435 — lexicon- en uitingeneditor
...
Om te voorkomen dat de browser alleen `index.html` vernieuwt maar een oude `viewer.js` houdt, laadt `index.html` nu `viewer.js?v4435` en `styles.css?v4435`.


## v4435 — OPN-slot voor vooropplaatsing
...
## v4435 — voorbeeldzinnen als HTML-input
...
## v4435 — structure-config als eerste stap
...
## v4435 — functional config: PRED apart, ARG-STRUCT apart
...
### v4435 startdiagnose
Open bij startproblemen `reset-cache.html?v4435` of `debug.html?v4435`. De debug controleert nu of de root-index daadwerkelijk de viewer is.



## v4435 — expliciete render-volgorde
...
## v4435 root-index fix
`docs/index.html` is vervangen door `docs/docs-home.html`; `viewer.html` is toegevoegd als fallback. Gebruik `debug.html?v4435` om te testen of root `index.html` echt de viewer is.

## v4435 — groei-presentatie
...
## v4435-correctie groei/projectiewissel

`LEX` is geen groei-projectie. In v4407 kon wisselen naar `LEX` de globale groeistap naar 0 zetten, waardoor `Assen`, `Bron` en `LOG/FT` leeg bleven zolang groei actief was. v4435 bewaart de laatst geldige groeistap en clamped alleen in ondersteunde groei-projecties.
