(() => {
  ...
  const els = {
    ...
    status: document.getElementById('carrouselStatus'),
  ...
  function show(i) {
    if (!slides.length) {
      ...
      els.text.textContent = 'Controleer carrousel/slides.json en carrousel/slides/.';
    ...
    if (slide.image) {
      ...
      els.image.alt = slide.title || 'JAN carrousel slide';
    }
  ...
  async function loadSlides() {
    try {
      ...
      setStatus(`slides.json niet geladen; fallback in carrousel.js gebruikt. ${err.message}`, true);
      return fallback;
    }
  }
  ...
  boot().catch(err => {
    els.title.textContent = 'Carrousel kon niet laden';
